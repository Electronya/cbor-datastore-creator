...
class TestAppWindow(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._MultiStateEditorCls = 'pkgs.ui.windows.appWindow.' \
            'MultiStateEditor'
        self._UintEditorCls = 'pkgs.ui.windows.appWindow.UintEditor'
    ...
    def test_displayEditorMultiState(self) -> None:
        """
        The _displayEditor method must create the multi-state editor when the
        selected node is a multi-state object.
        """
        selected = Mock()
        type = NodeType.MULTI_STATE
        objectEditor = Mock()
        with patch(self._MultiStateEditorCls) as mockedEditorCls:
            selected.getType.return_value = type
            mockedEditorCls.return_value = objectEditor
            self._uut._displayEditor(selected)
            mockedEditorCls.assert_called_once_with(selected)
    ...
    def test_displayEditorUint(self) -> None:
        """
        The _displayEditor method must create the uint editor when the
        selected node is a uint object.
        """
        selected = Mock()
        type = NodeType.UINT
        objectEditor = Mock()
        with patch(self._UintEditorCls) as mockedEditorCls:
            selected.getType.return_value = type
            mockedEditorCls.return_value = objectEditor
            self._uut._displayEditor(selected)
            mockedEditorCls.assert_called_once_with(selected)
